...
EEG dataset** (OpenNeuro, BIDS format with BrainVision files).

Mirrors the API of ``anphy_eeg.py`` and ``siena_eeg.py`` so that the
IgA pipeline can offload all raw-loading logic to a single call.

Typical workflow::

    # Low-level: direct path to .vhdr
    raw = load_test_retest_eeg(
        "/path/to/sub-01_ses-session1_task-eyesclosed_eeg.vhdr",
        t_start=0.0,
        t_stop=60.0,
        preload=False,
        verbose=True,
...
logger = logging.getLogger("test_retest_eeg")

# ---------------------------------------------------------------------------
# Channel whitelist (10-20 / 10-10 standard EEG channels)
# ---------------------------------------------------------------------------

_TEST_RETEST_VALID_EEG_CHANNELS: frozenset[str] = frozenset({
    # --- 10-20 core ---
...
def _resolve_vhdr_path(subject: str, session: str, task: str, db_path: str | Path) -> Path:
    """Build the .vhdr path from BIDS components."""
    ...
    vhdr = (
        ...
        / ses_dir
        / "eeg"
        / f"{subj_dir}_{ses_dir}_task-{task}_eeg.vhdr"
    )
    return vhdr
...
def load_test_retest_eeg(
    ...
    *,
    ...
    """
    Load a BrainVision ``.vhdr`` file from the test-retest dataset, keep
    only valid EEG/EOG channels, rename them to standard 10-20 names, set
    the montage, and optionally crop to a time window.
    ...
    Parameters
    ----------
    vhdr_file : str or Path
        Path to the ``.vhdr`` header file (MNE resolves ``.eeg`` + ``.vmrk``
    ...
    ------
    FileNotFoundError
        If ``vhdr_file`` or the companion ``.eeg`` / ``.vmrk`` do not exist.
    ValueError
        If no valid EEG channels are found or the crop window is invalid.
    """
    vhdr_file = Path(vhdr_file)
    if not vhdr_file.exists():
        raise FileNotFoundError(f"BrainVision header not found: {vhdr_file}")

    # Resolve companion files
    eeg_file = vhdr_file.with_suffix(".eeg")
    vmrk_file = vhdr_file.with_suffix(".vmrk")
    if not eeg_file.exists():
        raise FileNotFoundError(f"BrainVision data file not found: {eeg_file}")
    if not vmrk_file.exists():
        logger.warning("Marker file not found: %s", vmrk_file)
    ...
    # ------------------------------------------------------------------
    # Load BrainVision (preload=False keeps memory low)
    # ------------------------------------------------------------------
    raw = mne.io.read_raw_brainvision(str(vhdr_file), preload=preload, verbose=verbose)
    ...
    if _t_start is not None or _t_stop is not None:
        ...
        if verbose:
            print(f"  [TestRetest] Cropped to {start:.1f}s - {stop:.1f}s "
                  f"({(stop - start) / 60:.1f} min of "
                  f"{total_dur / 60:.1f} min total)")
    else:
        if verbose:
            print(f"  [TestRetest] Full recording loaded: {total_dur / 60:.1f} min")
    ...
    valid_chs = [
        ...
        if _clean_channel_name(ch) in _TEST_RETEST_VALID_EEG_CHANNELS
    ]

    if not valid_chs:
        all_chs = ", ".join(raw.ch_names)
        raise ValueError(
            f"No valid EEG channels found in {vhdr_file.name}. "
    ...
    if verbose:
        print(f"  [TestRetest] Kept {len(valid_chs)} EEG/EOG channels, "
    ...
    try:
        ...
        if verbose:
            print(f"  [TestRetest] Montage 'standard_1020' set successfully.")
    except Exception as exc:
        if verbose:
            print(f"  [TestRetest] Warning: could not set montage: {exc}")
...
def load_test_retest_eeg_from_ids(
    subject: str,
    session: str,
    task: str,
    *,
    ...
    and *task* identifiers, then calls :func:`load_test_retest_eeg`.
    ...
    db_path : str | Path | None
        Root of the BIDS dataset.  If ``None``, reads from the environment
        variable ``DB_TEST_RETEST_PATH`` or falls back to
        ``src.utils.config.DB_TEST_RETEST_PATH``.
    t_start, t_stop, t_minutes, first_n_minutes, preload, verbose
        Forwarded to :func:`load_test_retest_eeg`.
    ...
    ------
    FileNotFoundError
        If the resolved ``.vhdr`` does not exist.
    """
    # Resolve db_path
    if db_path is None:
        import os
        db_path = os.environ.get("DB_TEST_RETEST_PATH", "")
        if not db_path:
            try:
                from src.utils.config import DB_TEST_RETEST_PATH as _cfg_path
                db_path = _cfg_path
            except ImportError:
                raise FileNotFoundError(
                    "No db_path provided and DB_TEST_RETEST_PATH not set "
                    "in environment or config."
                )

    vhdr = _resolve_vhdr_path(subject, session, task, db_path)
    if not vhdr.exists():
        raise FileNotFoundError(
            f"Expected BrainVision file not found: {vhdr}\n"
            f"  (subject={subject}, session={session}, task={task}, "
            f"db_path={db_path})"
        )

    if verbose:
        print(f"  [TestRetest] Resolved: {vhdr}")

    return load_test_retest_eeg(
        vhdr,
        t_start=t_start,
        t_stop=t_stop,
        t_minutes=t_minutes,

#!/usr/bin/env python3
"""
run_batch_iga_test_retest_gedai.py
====================================
Ejecutor de batch para el pipeline de IgA sobre el dataset test-retest
preprocesado con **Gedai** (formato EEGLAB .set/.fdt).

Ejecuta corridas secuenciales o paralelas del pipeline
``test_iga_from_eeg_latent_test_retest_gedai.py`` según un archivo de parámetros
(batch_params_test_retest_gedai.txt), con soporte de checkpointing, logging CSV y
...
--------------------
TEST_RETEST_GEDAI_BATCH_PARAMS_FILE
...
Desde la raíz del proyecto::
    ...
    export TEST_RETEST_GEDAI_BATCH_PARAMS_FILE=/ruta/a/batch_params_test_retest_gedai.txt
    python -m src.batch_runs.run_batch_iga_test_retest_gedai
...
logger = logging.getLogger("batch_iga_test_retest_gedai")

logging_path = Path(LOGGING_BASE_PATH + "/batch_iga_test_retest_gedai") if LOGGING_BASE_PATH else None
if not logging_path:
    logger.warning("No se definió LOGGING_BASE_PATH; logs no se guardarán en archivo.")
else:
    os.makedirs(logging_path, exist_ok=True)

if os.environ.get("TEST_RETEST_GEDAI_LOG_LEVEL", LOGGING_LEVEL).upper() == "DEBUG":
...
PARAMS_FILE: Path = Path(
    _env("TEST_RETEST_GEDAI_BATCH_PARAMS_FILE", f"{BASE_PARAMS_FILE}/batch_params_test_retest.txt")
)
TEST_RETEST_GEDAI_DB_PATH: Path = Path(_env("TEST_RETEST_GEDAI_DB_PATH", DB_TEST_RETEST_GEDAI_PATH))
OUTPUT_DIR: Path = Path(_env("TEST_RETEST_GEDAI_OUTPUT_DIR", BASE_RESULTS_PATH))
CACHE_DIR: Path = Path(_env("TEST_RETEST_GEDAI_CACHE_DIR", BASE_CACHE_PATH))

CHECKPOINT_FILE: Path = CACHE_DIR / _env(
    "TEST_RETEST_GEDAI_CHECKPOINT_FILENAME", "batch_checkpoint_test_retest_gedai.json"
)

DELAY: float = _env_float("TEST_RETEST_GEDAI_DELAY", 2.0)
IGNORE_CACHE: bool = _env_bool("TEST_RETEST_GEDAI_IGNORE_CACHE", False)
RUN_POSTPROCESS: bool = _env_bool("TEST_RETEST_GEDAI_RUN_POSTPROCESS", True)
MAX_WORKERS: int = _env_int("TEST_RETEST_GEDAI_MAX_WORKERS", 1)

LATENT_DIM: int = _env_int("TEST_RETEST_GEDAI_LATENT_DIM", 2)
SCORING_METHOD: str = _env("TEST_RETEST_GEDAI_SCORING_METHOD", "hankel_dmd")
ICA_METHOD: str = _env("TEST_RETEST_GEDAI_ICA_METHOD", "picard")
L_FREQ: float = _env_float("TEST_RETEST_GEDAI_L_FREQ", 1.0)
...
class BatchRunnerTestRetestGedai:
    """Orquesta la ejecución del pipeline IgA sobre test-retest Gedai en batch."""
    ...
    def __init__(self) -> None:
        ...
        if not _PROJECT_ROOT.exists():
            ...
            sys.exit(1)
        if not TEST_RETEST_GEDAI_DB_PATH.exists():
            logger.warning("Ruta de test-retest Gedai no encontrada: %s", TEST_RETEST_GEDAI_DB_PATH)

        self.checkpoint: set[str] = self._load_checkpoint()

        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.log_file = logging_path / f"batch_log_test_retest_gedai_{ts}.csv"
        self._init_csv_log()

        logger.info("=" * 70)
        logger.info("  BATCH IgA -- Test-Retest Gedai (EEGLAB .set/.fdt)")
        logger.info("=" * 70)
        logger.info("  Params file : %s", PARAMS_FILE)
        logger.info("  Pipeline    : %s", PIPELINE_MODULE)
        logger.info("  DB path     : %s", TEST_RETEST_GEDAI_DB_PATH)
    ...
    def _run_single_job(self, row: list[str]) -> tuple[str, bool]:
        ...
        try:
            ...
            logger.error("No se encontró .set para %s/%s/%s: %s", subject, session, task, exc)
            return key, False
    ...
    def _run_postprocessing(self) -> None:
        ...
        logger.info("=" * 70)
        try:
            postprocess_dir = OUTPUT_DIR / "postprocess_asymmetry_gedai"
            run_postprocessing(
                db_name="test_retest_gedai",
                root_dir=OUTPUT_DIR,
...
def main() -> int:
    with warnings.catch_warnings():
        ...
        runner = BatchRunnerTestRetestGedai()
        return runner.run()
